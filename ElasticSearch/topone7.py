...
def senSelection(url, normclaim,verbs,entities):
#     """Simple Elasticsearch Query"""
#     # print(titles)
#     senquery=[]
    entityQuery = []
...
if __name__ == '__main__':
    ...
    for key, content in d.items():
        ...
        srlPredict=srlPredictor.predict_json({"sentence": normClaim})['verbs']
        idx = 0
        ...
        verbs=[verb['verb'] for verb in srlPredict]
        print(verbs)
        ...
        # #top one
        judge=''
        for candidate in senSelection(url,normClaim,verbs,entities)['hits']['hits']:
            sentence = candidate['_source']["sentence_text"]
