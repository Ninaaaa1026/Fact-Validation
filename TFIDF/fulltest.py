...
class process():
    ...
    def dataProcessing(self):
        ...
        files = os.listdir('./wiki-pages-text-pages-text/')
        vocab = Counter()
        doc_vocab = defaultdict(Counter)
        norm_docs = defaultdict(lambda:defaultdict())
        num_sentence = 0
        for f in files:
            with open('./wiki-pages-text-pages-text/'+ f, 'r', encoding='utf-8') as f:
                for raw_doc in f:
...
if __name__ == '__main__':
    ...
    # #load stored normdoc
    #     # with open('./normdoc.json', 'r', encoding='utf-8') as f:
    #     #     d = json.load(f)
    #     #     f.close()
    #     #
